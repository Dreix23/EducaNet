...
const updateTarea = async () => {
  ...
  const tareaDocRef = doc(db, collectionPath, tareaId.value);

  try {
    await updateDoc(tareaDocRef, tareaData);
    closeDialog();
  } catch (error) {
    ...
    <div v-if="isDialogOpen" class="fixed inset-0 flex items-center justify-center z-50">
      ...
      <div class="bg-white rounded-2xl shadow-lg p-8 w-full max-w-md z-10 relative">
        <div class="flex justify-between items-center mb-6">
          <h2 class="text-2xl font-bold text-blue-600 text-center flex-grow">Editar Tarea</h2>
        ...
        <form @submit.prevent="updateTarea">
          <div class="mb-6 relative">
            ...
            <input id="titulo" v-model="titulo" required placeholder="Titulo"
          ...
          <div class="mb-6 relative">
            <label for="fechaEntrega" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Fecha de
              Entrega</label>
