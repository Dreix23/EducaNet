...
const openDialog = (aviso) => {
  ...
  logDebug('Diálogo de editar aviso abierto');
};

const closeDialog = () => {
  isDialogOpen.value = false;
  resetForm();
  logDebug('Diálogo de editar aviso cerrado');
};
...
const updateAviso = async () => {
  ...
  try {
    ...
    const avisoDocRef = doc(schoolDocRef, 'avisos', avisoId.value);

    await updateDoc(avisoDocRef, avisoData);

    logInfo('Aviso actualizado con éxito');
    closeDialog();
  } catch (error) {
    logError('Error al actualizar el aviso: ' + error.message);
  } finally {
            ...
            <button type="submit"
                    ...
                    :disabled="isLoading">
                            <span v-if="isLoading" class="flex items-center">
                                <LucideLoader class="animate-spin mr-2" /> Actualizando...
                            </span>
