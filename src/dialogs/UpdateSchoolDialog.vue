        ...
        <div v-if="isDialogOpen" class="fixed inset-0 flex items-center justify-center z-50">
            ...
            <div class="bg-white rounded-2xl shadow-lg p-8 w-full max-w-md z-10 relative">
                <div class="flex justify-between items-center mb-6">
                    <h2 class="text-2xl font-bold text-blue-600 text-center flex-grow">Editar Colegio</h2>
                ...
                <form @submit.prevent="addSchool">
                    <div class="mb-6 relative">
                        ...
                        <input id="schoolCT" v-model="schoolCT" @input="schoolCT = toUpperCase($event.target.value)"
                            required placeholder="Código CT del Colegio"
                    ...
                    <div class="mb-6 relative">
                        <SchoolIcon class="absolute left-4 top-4 text-purple-400" />
                        <input id="schoolName" v-model="schoolName"
                            @input="schoolName = toUpperCase($event.target.value)" required
                            placeholder="Nombre del Colegio"
                            class="w-full pl-12 pr-3 py-4 text-gray-700 border border-color-gray-1 rounded-[15px] focus:outline-none  focus:border-color-gray" />
                    </div>
                    <div class="mb-6 relative">
                        <UserIcon class="absolute left-4 top-4 text-purple-400" />
                        <input id="directorName" v-model="directorName"
                            @input="directorName = toUpperCase($event.target.value)" required
                            placeholder="Nombre del Director"
                            class="w-full pl-12 pr-3 py-4 text-gray-700 border border-color-gray-1 rounded-[15px] focus:outline-none  focus:border-color-gray" />
                    </div>
                    <div class="mb-6 relative">
                        <MailIcon class="absolute left-4 top-4 text-purple-400" />
                        <input id="directorEmail" v-model="directorEmail" type="email" required
                            placeholder="Email del Director"
                            class="w-full pl-12 pr-3 py-4 text-gray-700 border border-color-gray-1 rounded-[15px] focus:outline-none  focus:border-color-gray" />
                    </div>
                    <div class="mb-6 relative">
                        <LockIcon class="absolute left-4 top-4 text-blue-400" />
                        <input id="directorPassword" v-model="directorPassword"
